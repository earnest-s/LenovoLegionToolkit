...
using LoqNova.CLI.Lib;
using LoqNova.CLI.Lib.Extensions;
using LoqNova.Lib;
...
using LoqNova.WPF.CLI.Features;

namespace LoqNova.WPF.CLI;
    ...
    )
{
    ...
    private async Task Handler(CancellationToken token)
    {
        try
        {
            ...
            await using var pipe = NamedPipeServerStreamAcl.Create(LoqNova.CLI.Lib.Constants.PIPE_NAME,
                PipeDirection.InOut,
                1,
